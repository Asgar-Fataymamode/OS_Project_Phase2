...
// // parse a command string into argv using simple whitespace splitting
...
//     // tokenize on spaces, tabs, and newlines using strtok (modifies input)
//     char* token = strtok(input, " \t\n");

//     // loop over tokens until none remain or capacity is almost reached
//     while (token && argc < MAX_ARGS - 1) {

//         // allocate space for the token plus null terminator
//         argv[argc] = malloc(strlen(token) + 1);
//         if (!argv[argc]) {

//             // free any previously allocated argument strings
